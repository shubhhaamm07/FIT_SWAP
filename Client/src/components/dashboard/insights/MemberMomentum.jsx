...
function MemberMomentum({ memberships = [] }) {
  ...
  const load = useCallback(async ({ signal } = {}) => {
    try {
      const [workoutData, mealData] = await Promise.all([getWorkoutPlan({ signal }), getMealLogs({}, { signal })]);
  ...
  useEffect(() => {
    ...
    const controller = new AbortController();
    if (!gymId) {
      const timer = window.setTimeout(() => setCrowd(null), 0);
      return () => { window.clearTimeout(timer); controller.abort(); };
    }
    const timer = window.setTimeout(() => {
      getGymCrowdLevel(gymId, { signal: controller.signal })
        .then((data) => { if (!controller.signal.aborted) setCrowd(data); })
        .catch((error) => { if (!controller.signal.aborted && !isRequestCancelled(error)) setCrowd(null); });
    }, 0);
    return () => { window.clearTimeout(timer); controller.abort(); };
