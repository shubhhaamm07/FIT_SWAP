...
function UpiPaymentCheckout({ request, busy = false, onMarkPaid, onCancel, compact = false, verificationNotice }) {
  ...
  const [secondsLeft, setSecondsLeft] = useState(() => Math.max(0, Math.ceil((new Date(request.expiresAt).getTime() - Date.now()) / 1000)));
  const isAwaitingPayment = request.status === "AWAITING_PAYMENT";
  const isMarkedPaid = request.status === "BUYER_MARKED_PAID";

  useEffect(() => {
    setSecondsLeft(Math.max(0, Math.ceil((new Date(request.expiresAt).getTime() - Date.now()) / 1000)));
    setUtr("");
  }, [request.id, request.expiresAt]);

  useEffect(() => {
    if (!isAwaitingPayment) return undefined;
    const interval = window.setInterval(() => setSecondsLeft(Math.max(0, Math.ceil((new Date(request.expiresAt).getTime() - Date.now()) / 1000))), 1000);
