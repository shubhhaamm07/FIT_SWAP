...
const PurchaseCard = ({ listing, onPurchased, isPurchased = false }) => {
  ...
  const handleCashRequest = async () => {
    ...
    try {
      setSubmitting(true);
      setMessage("");
      await createTransferRequest(listing.id);
      setCashRequested(true);
      setMessage(requiresGymApproval ? "Cash request sent. The seller must confirm it, then the gym owner will review the handover." : "Cash request sent. The seller must confirm the handover.");
    } catch (error) {
      setMessage(error.response?.data?.message || "Unable to create the cash transfer request.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (purchased || isPurchased) {
    return (
      <div className="sticky top-6 overflow-hidden rounded-2xl border border-emerald-400/25 bg-[#101a17] p-5 shadow-xl shadow-emerald-950/20">
        ...
        <p className="mt-2 text-sm leading-6 text-zinc-400">{requiresGymApproval ? "The seller confirmed your UPI payment and the gym approved the membership handover." : "The seller confirmed your UPI payment and the membership handover completed automatically."}</p>
  ...
  if (!onlinePaymentAllowed && !listing.allowCashTransfer) {
    return <div className="sticky top-6 rounded-2xl border border-amber-400/20 bg-[#18150f] p-5 shadow-xl shadow-black/20"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-amber-400/10 text-amber-300"><CircleAlert size={25} /></div><p className="mt-5 text-xs font-bold uppercase tracking-[0.16em] text-amber-300">Transfer unavailable</p><h2 className="mt-2 text-xl font-bold text-white">This plan has no available transfer method</h2><p className="mt-2 text-sm leading-6 text-zinc-400">Contact the gym for help with this membership.</p><button type="button" onClick={() => navigate("/marketplace")} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] py-3 font-semibold text-white transition hover:bg-white/[0.08]"><ArrowLeft size={17} />Back to marketplace</button></div>;
  }
  ...
  return (
    <div className="sticky top-6 rounded-2xl border border-violet-500/20 bg-[#12101b] p-5 shadow-xl shadow-black/20">
      <div className="flex items-center gap-2 text-violet-300"><ShieldCheck size={17} /><span className="text-xs font-semibold uppercase tracking-wider">{onlinePaymentAllowed ? "Protected UPI transfer" : "Cash handover request"}</span></div>
      <p className="mt-5 text-sm text-zinc-400">{onlinePaymentAllowed ? "Payable to seller" : "Listing price"}</p>
      <h2 className="mt-2 text-4xl font-bold text-white">{formatPrice(listing.price)}</h2>
      <div className="mt-5 border-t border-white/[0.08] pt-4 text-sm"><div className="flex justify-between text-zinc-400"><span>Membership value</span><span>{formatPrice(listing.price)}</span></div><p className="mt-3 text-xs leading-5 text-zinc-500">{onlinePaymentAllowed ? "FitSwap does not collect a platform or gym fee in the manual UPI pilot. You pay only the seller's listed amount." : "Confirm the cash payment directly with the seller, then complete each approval step in FitSwap."}</p></div>

      {onlinePaymentAllowed && (upiRequest ? <div className="mt-5"><UpiPaymentCheckout request={upiRequest} busy={submitting} onMarkPaid={handleMarkPaid} onCancel={upiRequest.status === "AWAITING_PAYMENT" ? handleCancelUpiRequest : undefined} compact /></div> : <button type="button" onClick={handleCreateUpiRequest} disabled={submitting || cashRequested} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-violet-600 py-3 font-semibold text-white transition hover:bg-violet-700 disabled:cursor-not-allowed disabled:opacity-60"><ShieldCheck size={18} />{submitting ? "Generating UPI QR…" : "Generate secure UPI QR"}</button>)}
      {listing.allowCashTransfer && <button type="button" onClick={handleCashRequest} disabled={submitting || Boolean(upiRequest) || cashRequested} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-amber-300/30 bg-amber-300/10 py-3 font-semibold text-amber-100 transition hover:bg-amber-300/20 disabled:cursor-not-allowed disabled:opacity-60">{cashRequested ? "Cash request sent" : "Request cash handover"}</button>}
      ...
      <div className="mt-6 rounded-xl bg-violet-500/10 p-4"><h3 className="font-semibold text-violet-300">What happens next?</h3><p className="mt-2 text-sm leading-6 text-zinc-400">Pay the exact QR amount, submit the UTR shown in your UPI app, then wait for the seller{requiresGymApproval ? " and gym" : ""} to confirm. The membership moves only after the policy checks are complete.</p></div>
    </div>
  );
};
