...
function CrowdLevelCard({ gymId }) {
  ...
  return <section className="overflow-hidden rounded-3xl border border-sky-400/20 bg-[radial-gradient(circle_at_2%_3%,rgba(14,165,233,.14),transparent_31%),radial-gradient(circle_at_92%_100%,rgba(124,58,237,.13),transparent_30%),#11131b] shadow-[0_18px_60px_rgba(0,0,0,.16)]">
    ...
    {loading ? <div className="flex min-h-72 items-center justify-center gap-2 text-sm text-zinc-400"><LoaderCircle size={17} className="animate-spin" /> Building crowd insights…</div> : <div className={`grid gap-0 ${crowd?.hasFullInsights ? "lg:grid-cols-[minmax(270px,.8fr)_minmax(320px,1.15fr)_minmax(300px,1fr)]" : "lg:grid-cols-[minmax(270px,.8fr)_minmax(420px,1.2fr)]"}`}><LivePulse current={current} crowd={crowd} sending={sending} onSend={send} message={message} liveTotal={liveTotal} />{crowd?.hasFullInsights ? <><WeeklyChart history={history} /><TimeChart history={history} /></> : <CrowdInsightsLocked />}</div>}
  </section>;
}

function CrowdInsightsLocked() {
  return <div className="relative overflow-hidden p-5 sm:p-6"><div className="absolute -right-12 -top-12 h-48 w-48 rounded-full bg-violet-500/15 blur-3xl" /><div className="relative max-w-md"><span className="grid h-10 w-10 place-items-center rounded-xl border border-violet-300/20 bg-violet-500/10 text-violet-200"><BarChart3 size={19} /></span><p className="mt-5 text-xs font-bold uppercase tracking-[0.14em] text-violet-300">FitSwap Plus</p><h3 className="mt-2 text-xl font-black tracking-tight text-white">Know the calmest day before you go.</h3><p className="mt-2 text-sm leading-6 text-zinc-400">Unlock the 30-day day-and-time crowd graphs, reliable quiet-time suggestions, and every gym&apos;s member activity pattern.</p><Link to="/plus" className="mt-5 inline-flex items-center gap-2 rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-violet-500"><SparkleIcon /> Unlock full crowd insights</Link><p className="mt-4 text-xs text-zinc-500">Live crowd reports remain free for every active member.</p></div></div>;
}

function SparkleIcon() { return <span aria-hidden="true" className="text-base leading-none">✦</span>; }
