...
const MyListingsPage = () => {
  ...
  const loadListings = async () => {
    try {
      setLoading(true);
      const [listingData, billingSummary] = await Promise.all([getMyListings(), getMyPlatformBilling()]);
  ...
  const handleRedeemPlusBoost = async (listing) => {
    try {
      setBoostingId(listing.id);
      const benefit = await redeemMemberListingBoost(listing.id);
      const until = new Intl.DateTimeFormat("en-IN", { day: "2-digit", month: "short", year: "numeric" }).format(new Date(benefit.benefitExpiresAt));
      setMessage(`Your complimentary FitSwap Plus boost is active until ${until}. This listing now receives priority placement.`);
      await loadListings();
    } catch (error) {
      setMessage(error.response?.data?.message || "Unable to activate your FitSwap Plus listing boost.");
    } finally {
      setBoostingId("");
    }
  };

  const hasPlus = Boolean(billing?.entitlements?.isFitSwapPlus);
  const plusBoostAvailable = Boolean(billing?.entitlements?.freeMonthlyBoostAvailable);
  ...
  return (
    <DashboardLayout rightSidebar={<MarketplaceSidebar />}>
      <main className="space-y-6">
        ...
        {hasPlus && (
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-emerald-400/20 bg-emerald-500/[0.06] px-4 py-3 text-sm">
            <div><span className="font-bold text-emerald-200">FitSwap Plus active</span><span className="ml-2 text-zinc-400">{plusBoostAvailable ? "Your complimentary 7-day priority boost is ready." : "Your complimentary priority boost has been used this month."}</span></div>
            <Link to="/plus" className="font-semibold text-emerald-200 hover:text-white">View Plus benefits</Link>
          </div>
        )}
        {message && (
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            {listings.map((listing) => (
              <ListingRow
                ...
                onRedeemPlusBoost={handleRedeemPlusBoost}
                isFitSwapPlus={hasPlus}
                plusBoostAvailable={plusBoostAvailable}
...
function ListingRow({ listing, onAction, onBoost, onRedeemPlusBoost, isFitSwapPlus, plusBoostAvailable, boosting }) {
    ...
    <article className="overflow-hidden rounded-xl border border-white/[0.08] bg-[#11121a] sm:flex">
      ...
      <div className="flex min-w-0 flex-1 flex-col p-4">
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          {listing.status === "ACTIVE" && isFitSwapPlus && (
            <button
              type="button"
              onClick={() => onRedeemPlusBoost(listing)}
              disabled={!plusBoostAvailable || boosting}
              className="inline-flex items-center gap-1 rounded-md border border-emerald-400/30 bg-emerald-500/[0.06] px-3 py-1.5 text-xs font-semibold text-emerald-200 transition hover:bg-emerald-500/15 disabled:cursor-not-allowed disabled:opacity-45"
              title={plusBoostAvailable ? "Use your monthly FitSwap Plus priority boost" : "Available once each calendar month with FitSwap Plus"}
            >
              <Sparkles size={13} /> {boosting && plusBoostAvailable ? "Activating…" : "Use Plus boost"}
            </button>
          )}
