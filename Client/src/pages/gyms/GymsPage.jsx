...
import { ArrowRight, BadgeCheck, Building2, CircleAlert, MapPin, MapPinned, Search, SlidersHorizontal } from "lucide-react";
...
function GymCard({ gym, onOpen }) { const primaryImage = gym.images?.find((image) => image.isPrimary) || gym.images?.[0]; const lowestPrice = gym.plans?.[0]?.price; return <article className="group overflow-hidden rounded-2xl border border-white/[0.08] bg-[#11121a] transition hover:-translate-y-1 hover:border-violet-400/35"><div className="relative h-44 overflow-hidden"><img src={primaryImage?.imageUrl || fallbackImage} alt="" className="h-full w-full object-cover opacity-70 transition duration-500 group-hover:scale-105 group-hover:opacity-90" /><div className="absolute inset-0 bg-gradient-to-t from-[#11121a] via-transparent" /><span className="absolute left-3 top-3 inline-flex items-center gap-1 rounded-full bg-emerald-500/90 px-2 py-1 text-[10px] font-bold text-white"><BadgeCheck size={12} /> Approved</span></div><div className="p-5"><h2 className="truncate text-lg font-bold text-white">{gym.name}</h2><p className="mt-1 flex items-center gap-1.5 text-sm text-zinc-400"><MapPin size={14} /> {gym.city}, {gym.state}</p><p className="mt-3 line-clamp-2 min-h-10 text-sm leading-5 text-zinc-500">{gym.description || "A verified FitSwap gym with official memberships."}</p><div className="mt-5 flex items-center justify-between border-t border-white/[0.07] pt-4"><div><p className="text-xs text-zinc-500">Plans from</p><p className="mt-1 font-bold text-white">{lowestPrice ? formatPrice(lowestPrice) : "View plans"}</p></div><button type="button" onClick={onOpen} className="inline-flex items-center gap-1.5 rounded-xl bg-violet-600 px-3 py-2 text-xs font-semibold text-white transition hover:bg-violet-500">View gym <ArrowRight size={14} /></button></div></div></article>; }
function EmptyGyms({ search }) { return <div className="mt-5 flex min-h-64 flex-col items-center justify-center rounded-2xl border border-dashed border-white/[0.1] bg-[#11121a] p-8 text-center"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-violet-500/10 text-violet-300"><Building2 size={22} /></div><h2 className="mt-4 font-semibold text-white">No gyms found</h2><p className="mt-2 text-sm text-zinc-500">{search ? "Try another gym name or location." : "Approved gyms will appear here."}</p></div>; }
