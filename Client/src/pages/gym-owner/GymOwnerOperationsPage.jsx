...
import OwnerGymsContent from "../../components/gym-owner/OwnerGymsContent";
import { RevenueByGymChart, RevenueTrendChart, SalesVolumeChart } from "../../components/gym-owner/OwnerRevenueCharts";
import { createGym, createMembershipPlan, getMyGyms, updateMembershipPlan, updateMyGym } from "../../api/gym.api";
...
function GymOwnerOperationsPage() {
  ...
  const onGymUpdated = async (gymData) => {
    try {
      ...
      const result = isNew ? await createGym(gymData) : await updateMyGym(editingGym.id, gymData);
      setEditingGym(null);
      setMessage(isNew
        ? "Gym added. Upload its photo PDF below to request admin approval."
        : result.status === "PENDING"
          ? "Profile saved. Submit an updated photo PDF if your location changed."
...
function BillingContent({ billing, updatingPaymentId, onStartPlan, onMarkPaid, onCancelPayment, onRefresh }) {
  ...
  useVisibilityPolling(async () => onRefresh(), {
    enabled: Boolean(pendingPaymentId && !["COMPLETED", "REJECTED", "CANCELLED", "EXPIRED"].includes(pendingPaymentStatus)),
    interval: 20_000,
    maxInterval: 120_000,
  });
