...
import { Ban, Building2, Check, CircleDollarSign, Clock3, CreditCard, LoaderCircle, Search, ShieldCheck, Store, UserCog, UserRoundCheck, UsersRound, X } from "lucide-react";

import DashboardLayout from "../../layouts/DashboardLayout";
import GymVerificationQueue from "../../components/admin/GymVerificationQueue";
...
import { confirmPlatformPayment, rejectPlatformPayment } from "../../api/platform-billing.api";
...
const sections = {
  ...
  payments: { eyebrow: "Payment operations", title: "Payment ledger", description: "Review manual UPI handovers alongside legacy online payment records across FitSwap.", icon: CircleDollarSign },
};
...
function AdminPortalPage() {
  ...
  const updateGym = async (gymId, status, review) => {
    try {
      setUpdating(gymId);
      await updateGymApproval(gymId, status, review);
      await load();
      setMessage(`Gym ${status.toLowerCase()} and owner notified.`);
    } catch (error) {
...
function PaymentLedger({ payments, updating, onConfirmPlatformFee, onRejectPlatformFee }) { const totalPaid = payments.filter((payment) => payment.status === "PAID" || payment.status === "COMPLETED").reduce((total, payment) => total + Number(payment.amount || 0), 0) / 100; return <section className="overflow-hidden rounded-2xl border border-white/[0.08] bg-[#11121a]"><div className="flex flex-col gap-3 border-b border-white/[0.08] p-5 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-semibold text-white">Payment handover activity</h2><p className="mt-1 text-sm text-zinc-500">Manual UPI requests, FitSwap service fees, and older online payment records.</p></div><div className="rounded-xl bg-emerald-500/10 px-4 py-2 text-right"><p className="text-[10px] font-bold uppercase tracking-wide text-emerald-300">Completed volume</p><p className="mt-1 text-lg font-bold text-white">{currency(totalPaid)}</p></div></div>{payments.length ? <div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="border-b border-white/[0.08] text-xs uppercase tracking-wide text-zinc-500"><tr><th className="px-5 py-4">Payment</th><th className="px-5 py-4">Buyer</th><th className="px-5 py-4">Amount</th><th className="px-5 py-4">Status</th><th className="px-5 py-4">Recorded</th><th className="px-5 py-4">Actions</th></tr></thead><tbody className="divide-y divide-white/[0.06]">{payments.map((payment) => { const needsPlatformReview = payment.provider === "PLATFORM_UPI" && payment.status === "BUYER_MARKED_PAID"; const paymentTitle = payment.provider === "PLATFORM_UPI" ? "FitSwap service fee" : payment.listing?.membership?.plan?.gym?.name || payment.plan?.gym?.name || "Gym membership"; const paymentDetail = payment.provider === "PLATFORM_UPI" ? `${payment.planCode?.replaceAll("_", " ") || "Platform feature"} · UPI · ${payment.paymentRef}` : `${payment.listing?.membership?.plan?.name || payment.plan?.name || "Membership transfer"} · ${payment.provider === "MANUAL_UPI" ? `UPI · ${payment.paymentRef}` : "Legacy online payment"}`; return <tr key={payment.id} className="transition hover:bg-white/[0.025]"><td className="px-5 py-4"><p className="font-medium text-white">{paymentTitle}</p><p className="mt-1 text-xs text-zinc-500">{paymentDetail}{payment.utr ? ` · UTR: ${payment.utr}` : ""}</p></td><td className="px-5 py-4"><p className="text-zinc-200">{name(payment.buyer)}</p><p className="mt-1 text-xs text-zinc-500">{payment.buyer?.email}</p></td><td className="px-5 py-4 font-semibold text-emerald-300">{currency(Number(payment.amount || 0) / 100)}</td><td className="px-5 py-4"><AdminStatus value={payment.status} /></td><td className="px-5 py-4 text-xs text-zinc-400">{date(payment.verifiedAt || payment.createdAt)}</td><td className="px-5 py-4">{needsPlatformReview ? <div className="flex flex-wrap gap-2"><button type="button" disabled={updating === payment.id} onClick={() => onConfirmPlatformFee(payment)} className="inline-flex items-center gap-1 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-semibold text-white hover:bg-emerald-500 disabled:opacity-50"><Check size={14} /> Confirm</button><button type="button" disabled={updating === payment.id} onClick={() => onRejectPlatformFee(payment)} className="inline-flex items-center gap-1 rounded-lg border border-red-500/30 px-3 py-2 text-xs font-semibold text-red-300 hover:bg-red-500/10 disabled:opacity-50"><X size={14} /> Reject</button></div> : <span className="text-xs text-zinc-600">—</span>}</td></tr>; })}</tbody></table></div> : <Empty title="No payment records yet" description="UPI handovers, FitSwap service fees, and legacy online payment history appear here." icon={CreditCard} />}</section>; }
function AdminStatus({ value }) { const styles = { APPROVED: "bg-emerald-500/10 text-emerald-300", ACTIVE: "bg-emerald-500/10 text-emerald-300", PAID: "bg-emerald-500/10 text-emerald-300", COMPLETED: "bg-emerald-500/10 text-emerald-300", PENDING: "bg-amber-500/10 text-amber-300", CREATED: "bg-amber-500/10 text-amber-300", AWAITING_PAYMENT: "bg-violet-500/10 text-violet-200", BUYER_MARKED_PAID: "bg-amber-500/10 text-amber-300", AWAITING_GYM_APPROVAL: "bg-sky-500/10 text-sky-300", PAUSED: "bg-sky-500/10 text-sky-300", REJECTED: "bg-red-500/10 text-red-300", CANCELLED: "bg-red-500/10 text-red-300", FAILED: "bg-red-500/10 text-red-300", EXPIRED: "bg-zinc-500/15 text-zinc-300" }; return <span className={`inline-flex rounded-full px-2 py-1 text-[10px] font-bold ${styles[value] || "bg-zinc-500/15 text-zinc-300"}`}>{String(value || "UNKNOWN").replaceAll("_", " ")}</span>; }
